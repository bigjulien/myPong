...
Game.prototype.populate = function(nbBalls){
    for (var i = 0 ; i < nbBalls ; ++i){
        var added = this.addBall(new Ball({ "name":i, "radius" : random(25,100), "position" : new randomPoint(0,0,this.field.width,this.field.height),"velocityVector" : new Vector(random(-3,3),random(-3,3)), "color" : (function(){var rand = Math.random(); return (rand<0.33)?"#F00":(rand<0.66)?"#0F0":"#00F";})()}));
        while (!added){
            added = this.addBall(new Ball({ "name":i, "radius" : random(25,100), "position" : new randomPoint(0,0,this.field.width,this.field.height),"velocityVector" : new Vector(random(-3,3),random(-3,3)), "color" : (function(){var rand = Math.random(); return (rand<0.33)?"#F00":(rand<0.66)?"#0F0":"#00F";})()}));
        }
    }
};
